...
const clanTagInput = document.querySelector("#clanTag");
const getClan = document.querySelector("#getClan");
...
getClan.addEventListener("click", async () => {
  if (!clanTag.value) {
    errorNotification.innerHTML = "Please add clan tag!";
    return;
  }
  try {
    getClan.classList.add("is-loading");
    getClan.disabled = true;
    ...
    clan.members.map((member) => {
      ...
      totals.allWinRate += parseInt(member.stats.allWinRate) || 0;
